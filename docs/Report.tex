...
For this comparison, gemma-3-4b-it was selected, an instruction-tuned model from Google’s Gemma family with approximately 4 billion parameters. Unlike the BERT baseline considered at this stage---which uses fixed BERT embeddings with a simple linear classifier (\texttt{LinearHead(768$\rightarrow$3)})---the LLM is evaluated through Few-Shot Prompting via the Google GenAI API, without any parameter updates.
...
\begin{enumerate}
    \item \textbf{Performance Gap:} The fine-tuned BERT model outperforms the LLM by approximately \textbf{3.6\% in Macro F1}. This confirms that, for this dataset, a smaller model adapted to the specific domain is more effective than a massive model relying on general knowledge (few-shot).
    
    \item \textbf{Resource Efficiency:} The efficiency gap is substantial. The BERT model (110M parameters) can be deployed for inference with low latency on standard hardware. In contrast, the LLM (4B parameters) requires significant VRAM or API dependence.
...
As illustrated in Fig.~\ref{fig:bert_input}, each input token is mapped to an embedding that combines three components: token embeddings (lexical identity), segment embeddings (sentence/segment membership), and position embeddings (token order). In addition, every sequence begins with the special classification token \texttt{[CLS]}. The final hidden state associated with \texttt{[CLS]} ($C \in \mathbb{R}^H$) is commonly used as an aggregate representation of the whole sequence for downstream classification. A separator token \texttt{[SEP]} is used to delimit (or separate) sequences when required.
...
\begin{figure}[!ht]
  ...
  \caption{Feature-based approach results on CoNLL-2003 NER.}
